...
use zksync_crypto::circuit::order::CircuitOrderTree;
use zksync_crypto::params::{account_tree_depth, order_tree_depth};
...
impl<DB: DatabaseInterface> WitnessGenerator<DB> {
    ...
    async fn load_order_tree(
        &self,
        block: BlockNumber,
    ) -> Result<CircuitOrderTree, anyhow::Error> { // TODO
        let mut circuit_order_tree = CircuitOrderTree::new(order_tree_depth());

        Ok(circuit_order_tree)
    }
    ...
    async fn load_account_tree(
        ...
        if let Some((cached_block, account_tree_cache)) = cache {
            let (_, accounts, orders) = self
                .database
                .load_committed_state(&mut storage, Some(block))
                .await?;
            for (id, account) in accounts {
                circuit_account_tree.insert((*id).into(), account.into());
            }
            ...
            if block != cached_block {
                ...
                let (_, accounts, orders) = self
                    ...
                    .load_committed_state(&mut storage, Some(block))
                    .await?;
                if let Some((_, account_updates, order_updates)) = self
                    .database
                    ...
                    .await?
                {
                    ...
                    for idx in updated_accounts {
                        circuit_account_tree
                            .insert((*idx).into(), accounts.get(&idx).cloned().unwrap_or_default().into());
                    }
                }
    ...
    async fn prepare_witness_and_save_it(&self, block: Block) -> anyhow::Result<()> {
        ...
        let mut circuit_order_tree = self.load_order_tree(block.block_number - 1).await?;
        metrics::histogram!("witness_generator", start.elapsed(), "stage" => "load_order_tree_full");

        let start = Instant::now();
        let witness: ProverData = build_block_witness(&mut circuit_account_tree, &mut circuit_order_tree, &block)?.into();
