...
async fn update_prover_job_queue<DB: DatabaseInterface>(database: DB) -> anyhow::Result<()> {
...

    {
        ...
        {
            let blocks_json = value.get("blocks").unwrap();
            let mut blocks = Vec::new();
            for block in blocks_json.as_array().unwrap() {
                let block_number = block.get("block_number").unwrap().as_u64().unwrap() as u32;
                let block_chunks_size = block.get("block_chunks_size").unwrap().as_u64().unwrap() as usize;
                blocks.push(CreateBlockProof { block_number: BlockNumber(block_number), block_chunks_size });
            }
