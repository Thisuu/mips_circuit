...
use zksync_types::{aggregated_operations::{AggregatedActionType, AggregatedOperation}, block::Block, prover::{ProverJob, ProverJobType}, AccountMap, AccountUpdates, BlockNumber, OrderMap, OrderUpdates};
// Local uses
...
impl DatabaseInterface for Database {
    ...
    ) -> anyhow::Result<(BlockNumber, AccountMap, OrderMap)> {
        let result = connection
            .chain()
            .state_schema()
    ...
    ) -> anyhow::Result<Option<(BlockNumber, AccountUpdates, OrderUpdates)>> {
        let result = connection
            .chain()
            .state_schema()
    ...
    async fn store_account_tree_cache(
        ...
        if block >= BlockNumber(NUMBER_OF_STORED_ACCOUNT_TREE_CACHE) {
            connection
                .chain()
                .block_schema()
